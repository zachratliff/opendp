...
use opendp::trans::{BoundedSumConstant};
use opendp::trans;
...
#[no_mangle]
pub extern "C" fn make_bounded_sum(type_args: *const c_char, lower: *const c_void, upper: *const c_void) -> *mut FfiResult<*mut FfiTransformation> {
    fn monomorphize<T>(type_args: Vec<Type>, lower: *const c_void, upper: *const c_void) -> FfiResult<*mut FfiTransformation>
        ...
        fn monomorphize2<MI, MO>(lower: MO::Distance, upper: MO::Distance) -> FfiResult<*mut FfiTransformation>
            ...
            trans::make_bounded_sum::<MI, MO>(lower, upper).into()
        }
    ...
    into_raw(dispatch!(monomorphize, [(type_output, @numbers)], (type_args, lower, upper)))
}


#[no_mangle]
pub extern "C" fn make_bounded_sum_n(type_args: *const c_char, lower: *const c_void, upper: *const c_void, n: c_uint) -> *mut FfiResult<*mut FfiTransformation> {
    fn monomorphize<T>(type_args: Vec<Type>, lower: *const c_void, upper: *const c_void, n: usize) -> FfiResult<*mut FfiTransformation>
        ...
        fn monomorphize2<MO>(lower: MO::Distance, upper: MO::Distance, n: usize) -> FfiResult<*mut FfiTransformation>
            ...
            trans::make_bounded_sum_n::<MO>(lower, upper, n).into()
        }
        let lower = try_as_ref!(lower as *const T).clone();
        let upper = try_as_ref!(upper as *const T).clone();
        dispatch!(monomorphize2, [
    ...
    let type_output = try_raw!(type_args[0].get_sensitivity_distance());
    into_raw(dispatch!(monomorphize, [(type_output, @numbers)], (type_args, lower, upper, n)))
}